...
{
    // LLMNR registry key read by the scanner
    ...
    // Revised PDF item 11 — seven distinct controls across three layers.
    private static readonly List<SubCheck> SubChecks = new()
    {
        // ── 11.1 LLMNR (the only one this check programmatically reads) ──
        new SubCheck
        {
            Id = "LLN-001.1",
            Title = "Turn off multicast name resolution (LLMNR)",
            Expected = "Enabled",
            ...
            DestinationLabel = "Administrative Templates > Network > DNS Client > Turn off multicast name resolution",
        ...
        // ── 11.2 NetBIOS over TCP/IP — preferred central method (DHCP) ──
        new SubCheck
        {
            Id = "LLN-001.2",
            Title = "NetBIOS over TCP/IP — disable via DHCP (preferred central method)",
            Expected = "0x2 (Disable NetBIOS) on scope options",
            ...
            ConsoleTool = "dhcpmgmt.msc",
            DestinationLabel = "DHCP > Scope Options > 001 Microsoft Disable Netbios Option = 0x2",
            ...
            GoTo = "Network Connections > Adapter > IPv4 > Advanced > WINS > 'Disable NetBIOS over TCP/IP'",
            GraphicalSteps =
                "1) Open dhcpmgmt.msc on the DHCP server.\n" +
        ...
        // ── 11.3 NetBIOS client prerequisite for DHCP-driven control ──
        new SubCheck
        {
            Id = "LLN-001.3",
            Title = "NetBIOS client prerequisite — adapter set to use DHCP value",
            Expected = "Use NetBIOS setting from the DHCP server",
            ...
            ConsoleTool = "ncpa.cpl",
            DestinationLabel = "Network Connections > Adapter > IPv4 > Advanced > WINS",
            ...
            GoTo = "Registry Editor > HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings > AutoDetect > 1",
            GraphicalSteps =
                "1) Run ncpa.cpl.\n" +
        ...
        // ── 11.4 NetBIOS fallback local adapter method ──
        new SubCheck
        {
            Id = "LLN-001.4",
            Title = "NetBIOS over TCP/IP — direct per-adapter disable (no DHCP control)",
            Expected = "Disable NetBIOS over TCP/IP on every adapter",
            ...
            ConsoleTool = "ncpa.cpl",
            DestinationLabel = "Network Connections > Adapter > IPv4 > Advanced > WINS > Disable",
            ...
            ConsolePath = "ncpa.cpl > adapter properties > IPv4 > Advanced > WINS",
            YouAreHere = "Network Connections (ncpa.cpl)",
            GoTo = "Adapter > Properties > IPv4 > Advanced > WINS > Disable NetBIOS over TCP/IP",
            GraphicalSteps =
                "1) Run ncpa.cpl.\n" +
        ...
        // ── 11.5 NetBIOS registry deployment method (central GPO/GPP) ──
        new SubCheck
        {
            Id = "LLN-001.5",
            Title = "NetBIOS over TCP/IP — registry deployment (central GPO/GPP)",
            Expected = "NetbiosOptions = 2 under every Tcpip_* interface",
            ...
            ConsoleTool = "gpmc.msc",
            DestinationLabel = "GPO > Preferences > Windows Settings > Registry > HKLM\\SYSTEM\\...\\Tcpip_*\\NetbiosOptions = 2",
            ...
            GoTo = "Preferences > Windows Settings > Registry > new item targeting Tcpip_* interfaces",
            GraphicalSteps =
                "1) Open gpmc.msc, edit the hardening GPO.\n" +
        ...
        // ── 11.6 WPAD (WinHTTP layer, central GPO/GPP) ──
        new SubCheck
        {
            Id = "LLN-001.6",
            Title = "WPAD — disable WinHTTP proxy auto-discovery (central)",
            Expected = "DisableWpad = 1",
            ...
            ConsoleTool = "gpmc.msc",
            DestinationLabel = "GPO > Preferences > Windows Settings > Registry > HKLM\\...\\WinHttp\\DisableWpad = 1",
            ...
            ConsolePath = "Computer Configuration > Preferences > Windows Settings > Registry",
            YouAreHere = "Group Policy Management (gpmc.msc) editing a GPO",
            GoTo = "Preferences > Windows Settings > Registry > WinHttp > DisableWpad = 1",
            GraphicalSteps =
                "1) Open gpmc.msc, edit the hardening GPO.\n" +
                "2) Computer Configuration > Preferences > Windows Settings > Registry.\n" +
                "3) Right-click > New > Registry Item.\n" +
            ...
            HasRegistryPath = true,
            RegistryPath = @"HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings\WinHttp\DisableWpad",
            AlternativeToRegistry = "Prefer central GPO/GPP deployment over local registry editing."
        },

        // ── 11.7 WPAD (user/browser layer) ──
        new SubCheck
        {
            Id = "LLN-001.7",
            Title = "WPAD — disable browser/user-layer auto-detect (optional hardening)",
            Expected = "Automatically detect settings = Off",
            ...
            ConsoleTool = "inetcpl.cpl",
            DestinationLabel = "Internet Options > Connections > LAN settings > Automatically detect settings = Off",
            ...
            GoTo = "Connections > LAN settings > uncheck 'Automatically detect settings'",
            GraphicalSteps =
                "1) Run inetcpl.cpl.\n" +
            ...
            HasRegistryPath = true,
            RegistryPath = @"HKCU\SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings\AutoDetect",
    ...
    public Task<Finding> EvaluateAsync()
    {
        ...
        try
        {
            ...
            // 0 = disabled (pass), 1 = enabled (fail), missing key = enabled by default (fail)
